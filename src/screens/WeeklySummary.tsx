  ...
  const Item = (athlete) => {
    return (
            ...
            <ListItem style={styles.Item} bottomDivider >
                ...
                <TotalScore points={athlete.athlete.Summary.Total} threshold="100" type="" />
             </ListItem>
            </View>
    )
  }
...
class WeeklySummary extends React.Component {
  ...
  state = { isLoaded: false, isFetching:false, weekly : [], yearly : [] } ; 
   
  refreshDataFromApi = () => {
      this.setState({ isFetching: true } );
      fetch( "https://api.everymovecounts.dk/athlete/summary?type=weekly")
        .then(response => response.json())
        .then(data => {
                 //console.log(data);
                 data.sort(f);
                 this.setState({ weekly : data, isLoaded: true, isFetching:false});
        }); 
  }
